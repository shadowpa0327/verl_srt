...
public:
    ...
    // Dump tree structure as a string for visualization/debugging.
    std::string dump() const;

    // Serialize tree to binary format for storage/network transfer.
    std::vector<uint8_t> create_snapshot() const;

    // Deserialize tree from binary format.
    static SuffixTree restore_snapshot(std::span<const uint8_t> data);
...
private:
    ...
    // Helpers for deserialization: rebuild sibling/group linked lists.
    static void _rebuild_siblings(Node* parent);
    static void _rebuild_all_siblings(Node* root);
};
