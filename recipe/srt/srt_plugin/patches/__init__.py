...
Usage:
    from recipe.srt.srt_plugin.patches import apply_all_patches
...
def apply_all_patches(force: bool = False) -> None:
    ...
    1. Config patches (SpeculativeConfig) - extends to accept suffix method
    ...
    # 1. Config patches - must be before other patches as they depend on config
    ...
    from recipe.srt.srt_plugin.patches import client_patches
    client_patches.apply_patches()

    # 5. LLM patches - depends on runner having drafter initialized
    from recipe.srt.srt_plugin.patches import llm_patches
