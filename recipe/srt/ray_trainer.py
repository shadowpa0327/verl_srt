...
class SRTRayDAPOTrainer(SRTRayPPOTrainer):
    """
    SRT + DAPO Trainer combining speculative rollout with DAPO's filter_groups.

    This trainer extends SRTRayPPOTrainer with DAPO-specific functionality:
    - filter_groups: Filters out prompts where all responses have the same reward (std=0)
    - Dynamic batch accumulation: Generates multiple batches until enough diverse prompts
    """

    def compute_kl_related_metrics(self, batch: DataProto, metrics: dict, timing_raw: dict):
        """Compute KL-related metrics (old_log_prob and ref_log_prob)."""
        batch.batch["response_mask"] = compute_response_mask(batch)
    ...
    def fit(self):
        """Training entry point - dispatches to appropriate DAPO implementation."""
        if self._should_use_runahead():
            return self._fit_dapo_runahead()
        return self._fit_dapo_standard()

    def _fit_dapo_runahead(self):
        """
        DAPO training loop with runahead sliding window and filter_groups support.

        Combines:
        - Runahead pattern: Primary batch (tick N) + Secondary batch (tick N+1) prefetch
        - DAPO filter_groups: Accumulates batches until enough diverse prompts (std > 0)

        The sliding window still advances, but filter_groups may require multiple
        primary generations before having enough prompts for training.
        """
        from verl.experimental.agent_loop.runahead import RunaheadConfig
        from verl.utils.tracking import Tracking
        ...
        print(f"SRT DAPO Runahead: Enabled with config: load_threshold={runahead_config.load_threshold}, "
              f"secondary_priority={runahead_config.secondary_priority}, "
              f"secondary_n={secondary_n} (primary_n={primary_n})")

        # add tqdm
        progress_bar = tqdm(total=self.total_training_steps, initial=self.global_steps, desc="Training (DAPO Runahead)")
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            ...
            while next_batch_dict is not None:
                ...
                # === PREPARE PRIMARY BATCH ===
                new_batch: DataProto = DataProto.from_single_dict(current_batch_dict)
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # SRT: Update suffix trees with rollout results
                    self._update_suffix_trees(new_batch, metrics, timing_raw)
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        accumulated_batch = new_batch if accumulated_batch is None else DataProto.concat([accumulated_batch, new_batch])
                        ...
                        if num_prompt_in_batch < prompt_bsz:
                            ...
                            if max_num_gen_batches <= 0 or num_gen_batches < max_num_gen_batches:
                                print(f"{num_gen_batches=}. Keep generating (runahead window continues)...")
                                self.gen_steps += 1
                                continue
                            else:
                                raise ValueError(
    ...
    def _fit_dapo_standard(self):
        """
        DAPO training loop with SRT integration and filter_groups support.

        This combines SRTRayPPOTrainer's suffix tree management with DAPO's
        dynamic sampling via filter_groups.
        """
        from verl.utils.tracking import Tracking
