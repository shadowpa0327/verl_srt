...
class AdmissionGateConfig:
    ...
    - waiting_threshold: Max num_requests_waiting to consider server has slack
...
class AdmissionControlledServer:
    ...
    def _has_slack(self, workload: dict[str, Any]) -> bool:
        ...
        if workload.get("warning") is not None:
            return False
        if "num_requests_waiting" not in workload or "kv_cache_usage" not in workload:
            return False
        try:
            waiting = int(workload["num_requests_waiting"])
            kv_cache = float(workload["kv_cache_usage"])
        except Exception:
            return False
        return waiting <= self._cfg.waiting_threshold and kv_cache <= self._cfg.kv_cache_threshold
...
class ServerSideAdmissionServerManager(AsyncLLMServerManager):
    ...
    def _has_slack(self, workload: dict[str, Any]) -> bool:
        ...
        if workload.get("warning") is not None:
            return False
        try:
            waiting = int(workload.get("num_requests_waiting", 999))
            kv_cache = float(workload.get("kv_cache_usage", 1.0))
        except (ValueError, TypeError):
            return False
        return (
            waiting <= self.admission_config.waiting_threshold
