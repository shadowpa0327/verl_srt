...
class RouterAdapter:
    """
    Adapter to make CentralRouter look like AsyncLLMServerManager.

    This allows existing agent loops (ToolAgentLoop, etc.) to work unchanged -
    they call `self.server_manager.generate()` which this adapter routes to the CentralRouter.

    Usage:
        adapter = RouterAdapter(router_handle)
        output = await adapter.generate(request_id, prompt_ids=..., sampling_params=...)
    """

    def __init__(self, router: ray.actor.ActorHandle):
        """Initialize the adapter.

        Args:
            router: Ray actor handle to CentralRouter.
        """
        self.router = router
    ...
    async def generate(
        ...
        *,
        ...
        """Route generate request through the CentralRouter.
        ...
        Returns:
            TokenOutput from the vLLM server.
        """
        return await self.router.generate.remote(
            request_id,
            prompt_ids=prompt_ids,
            sampling_params=sampling_params,
            image_data=image_data,
