#!/bin/bash
# =============================================================================
# Metrics Collection Sweep Script
# =============================================================================
# Iterates over combinations of benchmark parameters and collects vLLM metrics.
# Focuses on 1 round of execution for fast metrics collection.
#
# Usage:
#   ./scripts/run_metrics_sweep.sh                    # Run full sweep
#   ./scripts/run_metrics_sweep.sh --dry-run          # Preview commands only
#   ./scripts/run_metrics_sweep.sh --resume [DIR]     # Resume from DIR
#   ./scripts/run_metrics_sweep.sh --baseline-only    # Run only baseline configs
#   ./scripts/run_metrics_sweep.sh --runahead-only    # Run only runahead configs
#
# =============================================================================
...
# Multi-round settings (Optimized for metrics collection)
NUM_ROUNDS=1
WARMUP_ROUNDS=0
...
if [[ -n "$RESUME_DIR" ]]; then
    ...
    OUTPUT_DIR="results/metrics_sweep_${TIMESTAMP}"
fi
...
run_benchmark() {
    ...
    if [[ "$mode" == "baseline" ]]; then
        ...
        metrics_file="${OUTPUT_DIR}/metrics_baseline_ps${primary_size}_ltr${long_tail_ratio}.csv"
    else
        output_file="${OUTPUT_DIR}/runahead_ps${primary_size}_ltr${long_tail_ratio}_lt${load_threshold}_msc${max_secondary}.json"
        metrics_file="${OUTPUT_DIR}/metrics_runahead_ps${primary_size}_ltr${long_tail_ratio}_lt${load_threshold}_msc${max_secondary}.csv"
    fi
    ...
    cmd+=" --collect-metrics"
    cmd+=" --metrics-output $metrics_file"
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        if [[ $exit_code -eq 0 ]]; then
            log "DONE: Saved results to $output_file and metrics to $metrics_file"
        else
...
log "=========================================="
log "Metrics Collection Sweep"
log "=========================================="
log "Output directory: $OUTPUT_DIR"
log "Dry run: $DRY_RUN"
log "Resume: $RESUME"
log ""
log "Rounds: $NUM_ROUNDS measurement + $WARMUP_ROUNDS warmup"
log "=========================================="
log ""
...
log "=========================================="
log "Sweep complete!"
log "Results and metrics saved to: $OUTPUT_DIR"
log "=========================================="
